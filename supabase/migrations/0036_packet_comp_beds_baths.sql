-- Beds and baths on comparables, so an investor can judge whether a comp is
-- actually comparable rather than inferring it from square footage alone.

alter table public.packet_comps add column if not exists beds numeric;
alter table public.packet_comps add column if not exists baths numeric;
