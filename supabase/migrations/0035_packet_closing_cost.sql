-- Closing cost becomes an entered figure rather than a derived one.
--
-- It was computed as ARV x 10%. It is now set by hand and shown to investors as
-- a cost Bluebird absorbs on both sides of the transaction.
--
-- It is deliberately NOT part of the max offer, which stays on the flat rule:
--
--   max offer = ARV x 90% - 3 x repairs
--
-- so the ceiling and the disclosed closing figure are independent numbers.

alter table public.deal_packets add column if not exists closing_cost numeric;
