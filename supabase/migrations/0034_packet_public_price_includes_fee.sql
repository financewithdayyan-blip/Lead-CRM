-- The public purchase price now includes the assignment fee.
--
-- An admin enters what Bluebird pays (90k) and the fee (10k) separately; the
-- investor is quoted the combined 100k, because that is what they actually pay.
-- Combining server-side rather than in the client means the underlying 90k is
-- never in the public payload at all — adding it up in the UI would have left
-- the base price sitting in the JSON for anyone to read.
--
-- assignmentFee is still returned when show_assignment_fee is on, but it is now
-- a disclosed *component* of the quoted price rather than a figure on top of it.
