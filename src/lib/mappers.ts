import type { AttendanceSession, DailySummary, Lead, LeadActivity, LeadFile, LeadShare, Profile, Tag, TeamInvite, Task } from '@/types/domain';
...
export function dbToLead(row: any): Lead {
  return {
    ...
    auctionMilestonesNotified: row.auction_milestones_notified ?? [],
...
export function leadToDbInsert(lead: Partial<Lead>, userId: string) {
  return {
    ...
    auction_date: lead.auctionDate ?? null,
...
const LEAD_UPDATE_FIELDS: Array<[keyof Lead, string]> = [
  ...
  ['auctionMilestonesNotified', 'auction_milestones_notified'],
  ['condition', 'condition'],
...
export function dbToAttendanceSession(row: any): AttendanceSession {
  return { id: row.id, userId: row.user_id, startedAt: row.started_at, endedAt: row.ended_at };
}

export function dbToLeadShare(row: any): LeadShare {
  return {
    id: row.id,
    leadId: row.lead_id,
...
export function dbToTeamInvite(row: any): TeamInvite {
  return {
    id: row.id,
