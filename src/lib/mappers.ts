...
export function dbToLead(row: any): Lead {
  return {
    ...
    nextFollowUpTime: row.next_follow_up_time ? row.next_follow_up_time.slice(0, 5) : null,
    createdAt: row.created_at,
...
export function dbToCallingSession(row: any): CallingSession {
  return {
    id: row.id,
    userId: row.user_id,
    startedAt: row.started_at,
    endedAt: row.ended_at ?? null,
    callsLogged: row.calls_logged ?? 0,
  };
}
